...
def tickers(item):
	...
	item['oscrap_categories'] = list(set(categories))
...
def misc(item):
	...
	_authors.append(item.get("author_detail", default)['name'])
	...
	item['oscrap_authors'] = list(set(_authors))
	item['oscrap_credit'] = item.get('credit', '')
	item['oscrap_contributors'] = contribs
	item['oscrap_categories'].extend(item.get('dc_keyword', []))
...
def clean(item):
	...
	categories.extend(item.get('dc_keyword', []))
	...
	new_item = {
		...
		'timestamp' : timestamp,
		'oscrap_timestamp' : item['oscrap_acquisition_datetime'],
		'language' : language,
		'link' : item['link']
	}
